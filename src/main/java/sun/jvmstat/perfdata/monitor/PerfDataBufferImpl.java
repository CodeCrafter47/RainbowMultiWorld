...
public abstract class PerfDataBufferImpl {
	 ...
	 * @return Monitor - the {@link sun.jvmstat.monitor.Monitor} object that can be used to monitor the the named instrumentation object, or <tt>null</tt> if the named object doesn't exist.
	 ...
	 *
	 * This method returns a {@link java.util.List} of Monitor objects such that the name of each object matches the given pattern.
	 *
	 * @param patternString
	 *            a string containing a pattern as described in {@link java.util.regex.Pattern}.
	 * @return List<Monitor> - a List of {@link sun.jvmstat.monitor.Monitor} objects that can be used to monitor the instrumentation objects whose names match the given pattern. If no instrumentation objects have` names matching the given pattern, then an empty List is returned.
	 * @throws sun.jvmstat.monitor.MonitorException
	 *             Thrown if an error occurs while communicating with the target Java Virtual Machine.
